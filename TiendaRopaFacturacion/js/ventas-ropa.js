/**
 * 👗 Gestor de Ventas de Ropa - DE MODA CON VIVI
 * Integrado con la nueva estructura de base de datos
 */

class VentasRopaManager {
    constructor() {
        this.currentSale = {
            cliente: null,
            articulos: [],
    ...
    init() {
        ...
        this.renderSales();
        this.updateDisplay();
        console.log('👗 VentasRopaManager inicializado con base de datos');
    }

    loadData() {
        this.sales = window.database.getVentasRopaCompletas();
        this.clients = window.database.getAllClients();
        this.categories = window.database.getCategoriasRopa();
        this.products = window.database.getTable('ropa');
    ...
    renderProducts() {
        ...
        const categoryProducts = this.products.filter(p => p.idCategoriaRopa === this.currentCategory);
        ...
        categoryProducts.forEach(product => {
            ...
            productCard.innerHTML = `
                ...
                <div class="product-actions">
                    ...
                    <button class="btn btn-sm btn-primary" onclick="ventasRopaManager.addArticulo(${product.id})">
                        Agregar
                    </button>
                </div>
            `;
    ...
    finalizeSale() {
        ...
        try {
            ...
            this.currentSale.articulos.forEach(articulo => {
                ...
                const venta = window.database.createVentaRopa(ventaData);
            ...
            const factura = window.database.createFacturaRopa(facturaData);
        ...
        itemsContainer.innerHTML = this.currentSale.articulos.map(item => `
            <div class="sale-item">
                ...
                <div class="item-price">
                    ...
                    <button class="btn btn-sm btn-danger" onclick="ventasRopaManager.removeArticulo(${item.id})">
    ...
    groupSalesByInvoice() {
        const facturas = window.database.getTable('factura_ropa');
        const ventas = window.database.getTable('ventas_ropa');
        ...
        row.innerHTML = `
            ...
            <td>
                <div class="btn-group" role="group">
                    <button class="btn btn-sm btn-info" onclick="ventasRopaManager.viewArticulos(${factura.funda})" title="Ver artículos">
                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                            <path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path>
                            <circle cx="12" cy="12" r="3"></circle>
                        </svg>
                    </button>
                    <button class="btn btn-sm btn-primary" onclick="ventasRopaManager.printSale(${factura.funda})" title="Imprimir">
                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
...
document.addEventListener('DOMContentLoaded', function() {
    window.ventasRopaManager = new VentasRopaManager();
});
